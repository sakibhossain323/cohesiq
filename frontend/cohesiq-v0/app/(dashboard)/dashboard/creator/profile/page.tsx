...
import { getMyCreatorProfile, updateSocialProfile } from "@/lib/api/creators";
...
export default function CreatorPlatformsPage() {
  ...
  const [profiles, setProfiles] = useState<CreatorSocialProfile[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setProfiles(creator.social_profiles ?? []);
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (dialogMode === "add") {
        ...
        const data = await fetchApi<CreatorSocialProfile>(
          `/creators/${creatorId}/platforms`,
          { method: "POST", token, body: JSON.stringify(payload) }
        );
        setProfiles(prev => [...prev, data]);
        toast({ title: "Platform added", description: `${getPlatformLabel(form.platform)} connected successfully.` });
      } else if (dialogMode === "edit" && form.id) {
        const data = await updateSocialProfile(creatorId, form.id, token, payload);
        setProfiles(prev => prev.map(p => p.id === form.id ? data : p));
        toast({ title: "Platform updated", description: `${getPlatformLabel(form.platform)} details saved.` });
      }
      
      setShowDialog(false);
    } catch (err: any) {
      toast({ title: "Failed to save platform", description: err.message, variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleDelete = async (platformId: string, platformName: PlatformType) => {
    setDeletingId(platformId);
    try {
      const token = await getToken();
      if (!token || !creatorId) return;
      await fetchApi(`/creators/${creatorId}/platforms/${platformId}`, {
        method: "DELETE",
        token,
      });
  ...
  const handleSync = async (platformId: string, platformName: PlatformType) => {
    ...
    try {
      const token = await getToken();
      if (!token || !creatorId) return;
      // Simulate sync request which doesn't exist yet
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <DialogFooter className="pt-4 border-t">
            ...
            <Button
              onClick={handleSave}
              disabled={!form.platform || !form.handle || isSubmitting}
            >
