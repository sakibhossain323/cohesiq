...
import { getCampaignById, getCampaignMatches, runCampaignMatching } from "@/lib/api/campaigns";
...
export default function CampaignMatchesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const matchesData = await getCampaignMatches(campaignId, token);
      // Sort matches descending by total score
  ...
  useEffect(() => {
    ...
    loadData();
  }, [isLoaded, isSignedIn, campaignId]);
  ...
  const handleRunMatching = async () => {
    ...
    try {
      ...
      const result = await runCampaignMatching(campaignId, token);
      const sortedMatches = (result || []).sort((a, b) => (b.score_total || 0) - (a.score_total || 0));
      setMatches(sortedMatches);
    } catch (err: any) {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground animate-pulse">Analyzing matches for your campaign...</p>
      </div>
    );
  }
        ...
        <Card className="border-red-200 bg-red-50 dark:bg-red-950/20">
          <CardContent className="pt-6">
            <div className="flex items-start gap-3 text-red-800 dark:text-red-300">
              ...
              <div>
                <h3 className="font-semibold text-lg">Unable to load campaign matches</h3>
                <p className="mt-1 text-sm">{error || "Campaign not found"}</p>
                <div className="mt-6 flex gap-3">
                  <Button variant="outline" size="sm" onClick={() => router.back()} className="bg-background">
                    <ChevronLeft className="mr-1 h-4 w-4" /> Go Back
                  </Button>
                  <Button size="sm" onClick={loadData} className="bg-red-800 hover:bg-red-700 text-white">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {isMatching ? "Matching Engine Running..." : "Run AI Matching"}
          </Button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {matches.length === 0 ? (
          <Card className="border-dashed border-2 py-12 flex flex-col items-center justify-center text-center">
            ...
            <Button onClick={handleRunMatching} disabled={isMatching} className="mt-6">
              <Brain className="h-4 w-4 mr-2" />
              {isMatching ? "Running Engine..." : "Analyze & Score Creators"}
            </Button>
          </Card>
        ) : (
