...
| D15 | (not in SRS) AI-usage infrastructure layer | **Added on branch `sakib/ai-score-max`, all behind Compose `profiles:`** — (a) `cohesiq-matching-mcp` FastMCP server (`backend/mcp_server.py`, stdio+HTTP) exposing the matching engine as MCP tools; (b) Admin AI Assistant, a LangChain/LangGraph agent (`backend/app/admin/assistant.py`, `POST /admin/assistant`) that consumes the MCP server via `langchain-mcp-adapters`; (c) Prometheus `/metrics` + custom `cohesiq_*` counters + Grafana dashboard (`ops/`); (d) Ollama/Hermes local-LLM stub (`local-llm` profile); (e) n8n automation stub (`automation` profile). | **Additive, demo-safe, real.** Built to lift the BuildFest "AI Detail Usage" score with genuine implementations (not claims). Zero DB-schema change → no migration. Default `docker compose up` is unaffected; all new services are opt-in. The matching pipeline stays deterministic — the agent is for open-ended admin Q&A only. Config handbook: `docs/submission-actions.md`. |
