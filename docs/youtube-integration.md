...
> YouTube component reference. Validated on 2026-06-10.
...
Migration `0017` restores `UNIQUE(creator_id, platform)` on `creator_social_profiles`, which is required by the enrichment and seeding upserts. (This is the historical migration that introduced the constraint; the current Alembic head is `0022` — always run `alembic upgrade head` rather than targeting `0017`.)
...
Run after migrations are at the current head (`0022`; `0017` was the migration that first introduced the required `UNIQUE(creator_id, platform)` constraint):
